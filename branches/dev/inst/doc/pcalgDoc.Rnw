...
\code{pc.cons.internal()} with argument \code{version.unf = c(1,2)} after
computing the skeleton. This is a slight variation of the conservative PC
algorithm (which used \code{version.unf = c(2,2)}): If $V_a$ is independent
of $V_c$ given some $V_S$ in the skeleton (i.e., the edge $a \gredge{-} c$ dropped out),
but $V_a$ and $V_c$ remain dependent given all subsets of the adjacency set of
either $a$ or $c$, we call $a \gredge{-} b \gredge{-} c$ 
...
\code{conservative[2]} is \code{TRUE}), we call \code{pc.cons.internal(...,
  version.unf = c(1,2))} again after Possible-D-SEP was found and the graph
...
rankings. In Figure~\ref{fig:yeast} one can see that \code{ida()} is
clearly superior to the alternative methods (and random guessing) in terms
of identifying effects in the target set.
