...
structure from observational data. In particular, we discuss 
the algorithms PC 
(see \cite{SpirtesEtAl00})), FCI (see \cite{SpirtesEtAl00} and
\cite{SpirtesMeekRichardson99}), RFCI (see \cite{rfci}), GES (see 
\cite{Chickering2002}) and GIES (see \cite{HauserBuhlmann2012}).
...
(Th. 1.4.1) that there is a link between causal structures and graphical
...
For the causal inference algorithms based on conditional independence tests presented so far, you first had to calculate a sufficient statistic and to specify a conditional independence test function.  For the score-based GES algorithm, you have to define a score object instead, and then apply the inference algorithm.
