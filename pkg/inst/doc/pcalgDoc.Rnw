...
observational data. Under some assumptions, the PC algorithm
(see \cite{SpirtesEtAl00}), the FCI algorithm (see \cite{SpirtesEtAl00} and
\cite{SpirtesMeekRichardson99}) and the RFCI algorithm (see \cite{rfci}) can
infer information about the causal structure from observational data. Thus,
these algorithms tell us which variables could or could not be a cause of
...
\end{eqnarray*}
where $do(V_6 = x)$ denotes Pearl's do-operator (see \cite{Pearl00}). If
...
the PC algorithm
(see \cite{SpirtesEtAl00})), the FCI algorithm (see \cite{SpirtesEtAl00} and
\cite{SpirtesMeekRichardson99}) and the RFCI algorithm (see \cite{rfci}).
...
a graph consisting of nodes and directed edges ($\leftarrow$ or
$\rightarrow$). As a further restriction, the edges must be directed in a
...
following, we only deal with distributions where the following holds: For
each distribution, it is possible to find a DAG, whose list of d-separation
relations perfectly matches the list of conditional independencies of the
distribution. Such distributions are called faithful. It has been shown
...
contains the following types of edges: o--o, o--, o-->, -->, <-->,
...
by $P[V_y | do(V_x=x)]$. Note that this is different from the conditional
...
a randomly chosen hour, we get $P(V_x=1|do(V_y=1)) = P(V_x=1) = 0.1$. Thus, the
...
E[V_y|do(V_x=x)]$, as a general measure for the causal effect of $V_x$ on
$V_y$. For multivariate Gaussian random variables, $E[V_y|do(V_x=x)]$ depends
...
$E[V_y|do(V_x=x+1)] - E[V_y|do(V_x=x)]$. For binary random variables (with domain
$\{0,1\}$) we define the causal effect of $V_x$ on $V_y$ as $E(V_y|do(V_x=1)) -
E(V_y|do(V_x=0)) = P(V_y=1|do(V_x=1)) - P(V_y=1|do(V_x=0))$.
...
all unshielded triplets $a-b-c$ are checked in the following way. We test
...
of $V_c$ given some $V_S$ in the skeleton (i.e., the edge $a-c$ dropped out),
but $V_a$ and $V_c$ remain dependent given all subsets of the adjacency set of
either $a$ or $c$, we call $a-b-c$ \emph{``unambiguous''}. This is because
...
undirected edges $2-3-5$ can be oriented as a new v-structure. As
...
$2-3-5$ can only be oriented as $2 \rightarrow 3 \rightarrow
5$, $2 \leftarrow 3 \leftarrow 5$ or $2 \leftarrow 3
\rightarrow 5$, and not as $2 \rightarrow 3 \leftarrow 5$. There is
only one remaining undirected edge ($1-6$), which can be oriented in
...
\code{lm}($Vx \sim VS$) and \code{lm}($Vy \sim VS$), get the residuals, and
