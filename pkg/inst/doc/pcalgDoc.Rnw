...
(IDA) to obtain bounds on total causal effects based on observational data
under some assumptions (see \cite{BuKaMa09}). Furthermore, we presented an
...
\end{description}
\emph{Maximal Ancestral Graphs (MAG)}, which we will use from now on, also
obey a third rule:
\begin{description}
...
no directed path from node $i$ to node $j$ in the underlying DAG. If, on
the other hand, there is an arrowtail at node $i$, then there is a directed
path from node $i$ to node $j$ in the underlying DAG.
...
\emph{Partial Ancestral Graph (PAG)} (see
...
estimate is consistent under some assumptions by \cite{BuKaMa09}.
...
plot(pag.est)
@ In Fig.~\ref{fig:fci} the true DAG (left) and the estimated PAG (right)
are shown.
\begin{figure}
  \centering
