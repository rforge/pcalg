...
\Abstract{ The \pkg{pcalg} package for \proglang{R} (\cite{citeR}) can be
  used for the following two pruposes: Causal structure learning and
...
\Keywords{IDA, PC, RFCI, FCI, do-calculus, causality, graphical models, \proglang{R}}
\Plainkeywords{IDA, PC, RFCI, FCI, do-calculus, causality, graphical models, R} %% without formatting
...
observational data. Under some assumptions, the PC algorithm (XXX), the FCI
algorithm (XXX) and the RFCI algorithm (XXX) can infer information about
the causal structure from observational data. Thus, these algorithms tell
us which variables could or could not be a cause  of some variable of
interest. They do not, however, give information about the size of the
causal effects. We therefore developed the IDA method (see
...
contains implementations of the PC-, FCI- and RFCI-algorithms, as well as
...
should be $\leftarrow$ or $\rightarrow$. Thus, bidirected edges
...
\end{eqnarray*}
where $do(V_6 = x)$ denotes Pearl's do-operator (XXX). If the causal
relationships are linear, these two expressions are equivalent and do not
depend on $\tilde{x}$.
...
structure from observational data. In particular, we discuss the
PC-algorithm (XXX), the FCI-algorithm (XXX) and the RFCI-algorithm
(XXX). In Section \ref{sec:bounds} we describe the IDA method (XXX) to
...
model. The physical map here is a graph consisting of nodes and directed
edges ($\leftarrow$ or $\rightarrow$). As a further
restriction, the edges must be directed in a way, so that it is not
possible to trace a cycle when following the arrowheads (i.e. no directed cycles). The
interpretation rule is called d-separation. This rule is a
bit intricate and we refer the reader to \cite{lauritzen} for more
details. This interpretation rule can be used in the
following way: If two nodes $X$ and $Y$ are d-separated
...
$S$. For the following, we only deal with distributions where the
...
We now describe the PC-algorithm, which is shown in Algorithm
...
$G$ that are adjacent to node $i$) . If there is any node $k$ such that $i$
and $j$ are conditionally independent given $k$, the edge between $i$ and
...
variable can be visualized using a DAG where the edges indicate
direct cause-effect relationships.
Under some assumptions, \cite{Pearl00} showed (Th. 1.4.1) that there is a
link between causal structures and graphical models. Roughly speaking, if
the underlying causal structure is a DAG, we observe data generated from
this DAG and then estimate a DAG model (i.e. a graphical model) on this
data, the estimated CPDAG represents the equivalence class of the DAG model
describing the causal structure. This holds if we have enough samples and
assuming that the true underlying causal structure is indeed a DAG without
latent or selection variables. Note that even given an infinite amount of
data, we usually cannot identify the true DAG itself, but only its
equivalence class. Every DAG in this equivalence class is can be the true causal structure.
...
undirected (-) or bidirected ($\leftrightarrow$) (note that in the context of ancestral graphs,
...
underlying DAG (i.e., $i$ is a cause of $j$ or of the selection
...
partial ancestral graph (PAG) (see e.g.
...
XXXRichardson). The orientation rules of this algorithm were slightly
...
CITEcolombo for a detailed discussion of the FCI algorithm. It turns out
...
the street during some hour. The random variable $X$ denotes whether it rained during that
hour ($X=1$ if it rained, $X=0$ otherwise). The random variable $Y$ denotes
whether the street was wet at the end of that hour ($Y=1$
if it was wet, $Y=0$ otherwise). If we assume $P(X=1) = 0.1$ (rather dry
region), $P(Y=1|X=1) = 0.99$ (the street is almost always still wet at the
end of the hour when it rained during that hour) and $P(Y=1|X=0) = 0.02$
...
E[Y|do(X=x)]$, as a general measure for the causal effect of $X$ on
$Y$. For multivariate Gaussian random variables, $E[Y|do(X=x)]$ depends
...
$E[Y|do(X=x+1)] - E[Y|do(X=x)]$. For binary random variables (with domain
$\{0,1\}$) we define the causal effect of $X$ on $Y$ as $E(Y|do(X=1)) -
E(Y|do(X=0)) = P(Y=1|do(X=1)) - P(Y=1|do(X=0))$.
...
very useful when the multi-set has certain properties (e.g. all values are much
...
\item[FCI-algorithm:] Allows for hidden and selection variables; consistent
  ...
  XXX) are sparse, the data is multivariate Normal and satisfies some regularity
...
and reliable implementations of the PC, FCI and RFCI algorithms for
...
\code{fci()} and \code{rfci()} are intended for estimating graphical
...
\code{fixedGapgs} (absent edges) and \code{fixedEdges} (present edges).
...
information about the edge $b-c$, since it should be oriented as $b
\leftarrow c$ in v-structure $a \rightarrow b \leftarrow c$, while it
should be oriented as $b \rightarrow c$ in v-structure $b \rightarrow c
...
algorithm (CITE) is chosen. The conservative PC is a slight variation of
the PC algorithm and is intended to be more robust against sampling errors
in its edge orientations. After the skeleton is computed, all unshielded
triplets $a-b-c$ are checked in the following way. We test whether $a$ and
$c$ are conditionally independent given any subset of the adjacency set of
$a$ or any subset of the adjacency set of $c$. If $b$ is in no such
conditioning set (and not in the original sepset) or in all such
...
$c$ such that $a$ and $c$ are conditionally independent given $S$, the
...
whether $a-b-c$ is a v-structure or not are only applied to faithful
...
form o--o. Now, the v-structures are oriented as in the PC
...
FCI algorithm which consists of two parts. In the first part (done if
...
conservative PC algorithm (which used \code{version.unf = c(2,2)}): If $a$ is
...
neighbors of either $a$ or $c$, we call $a-b-c$
...
CITE showed that the algorithm is identical to the original FCI algorithm
...
PAG estimated with \code{fci()} are shown. Node $V_1$ is latent. We can
read off that both $V_4$ and $V_5$ cannot be a cause of $V_2$ and $V_3$,
which can be confirmed in the true DAG.
...
\caption{True underlying DAG (left) and estimated PAG
  ...
  \code{gmL}. The ouput of FCI and RFCI is identical. Variable 1 of the
...
A note on implementation: As \code{pc()}, \code{rfci()} and \code{fci()}
...
undirected edges $V_2-V_3-V_5$ can be oriented as a new v-structure. As
...
$V_2-V_3-V_5$ can only be oriented as $V_2 \rightarrow V_3 \rightarrow
V_5$, $V_2 \leftarrow V_3 \leftarrow V_5$ or $V_2 \leftarrow V_3
\rightarrow V_5$, and not as $V_2 \rightarrow V_3 \leftarrow V_5$. There is
only one remaining undirected edge ($V_1-V_6$), which can be oriented in
...
regression coefficient of $x$ in the regression \code{lm(y ~ x +
  pa(x))}, where \code{pa(x)} denotes the parents of $x$ in the DAG G (z is
called a parent of x if G if G contains the edge z $\rightarrow$ x); if
...
In some applications, it is clear that some variable is definitivly not an
effect or not. Consider for example a bacterium producing
...
\code{lm}($x \sim S$) and \code{lm}($y \sim S$), get the residuals, and
...
(CITE), biology (CITE) and the social sciences (CITE). We will discuss two
...
was to explore how graphical model can be
...
genes (CITE), the goal was to identify the largest intervention effects
between all pairs of genes. For the analysis, the \pkg{pcalg} package was
used.

CITE also provide gene expression measurements from 234 interventional
experiments, namely from 234 single-gene deletion mutant strains. Using
this data, we know the true causal effect of the knock-out genes on the
remaining genes in good approximation. We can
then quantify how well we can find the true intervention effects in the
following way: We encode the largest 10\% of the intervention effects
computed from the interventional data as the target set of effects that we
want to identify. We then check in an ROC curve, how
well the ranking of the causal effects estimated by applying \code{ida()}
to the observational data is able to identify effects in the target set. For comparison, the authors also used the (conceptually
wrong) Lasso and Elastic Net to obtain rankings. In
Figure \ref{fig:yeast} one can see that \code{ida()} is clearly superior to
the alternative methods (and random guessing) in terms of identifying
effects in the target set.
...
causal structure for the yeat data is likely to contain many sampling
