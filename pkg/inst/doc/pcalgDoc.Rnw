...
%% almost as usual
\newcommand*{\AUT}[1]{{\small #1} \\ {\small ETH Zurich}}
\author{
...
of an intervention. For example, suppose, we would increase variable $V_1$
by external intervention by one unit. The recorded increase in variable
$V_6$ is the (total) causal effect of $V_1$ on $V_6$. More precisely, the
causal effect $C(V_1, V_6, \tilde{x})$ of $V_1$ from $V_1 = \tilde{x}$ on $V_6$
is defined as
\begin{eqnarray*}
...
\Sexpr{round(causalEffect(gGtrue, 6, 1),2)} for variables
$V_4$, $V_5$ and $V_6$, respectively. The first row, corresponding to
variable $V_4$, quite accurately indicates a causal effect that is very
close to zero or no effect at all. The second row of the output,
corresponding to variable $V_5$, is rather uninformative: Although one entry
comes close to the true value, the other estimate is 0. Thus, we cannot be
sure if there is a causal effect at all. The third row, corresponding to
$V_6$ was already discussed above.
...
(\code{disCItest}), and binary data (\code{binCItest}). Each of
these independence test functions needs different arguments as input,
described in the respective help files. For example,
...
\code{gmG} (which consists of $p=8$ variables and $n=5000$
...
@
\caption{True DAG and estimated skeleton fitted on the discrete data set
  provided by the package using function \code{skeleton()}.}
\label{fig:skel2}
\end{figure}
...
arbitrary DAG is generated on the skeleton as with \code{u2pd = "rand"}.
...
\emph{``unfaithful''}.  Moreover, if in the conservative step there is no subset
among the neighbors that makes $a$ and $c$ independent, the triple is
marked \emph{``unfaithful''}. An unfaithful triple is not oriented as a v-structure.
...
\emph{``faithful''}. This is because in the FCI, the true separating set might be
...
Fig.~\ref{fig:ida}. Denote node $i$ in the graph by $V_i$. We assume that
...
V_3 \leftarrow V_5$ or $V_2 \leftarrow V_3 \rightarrow V_5$ but not as $V_2
...
(\code{V\textit{i}} denotes node $V_i$) for each
...
(this could be given by experts or estimated using \code{pc()}).
...
The functions \code{skeleton}, \code{pc} and \code{fci} all need as
\emph{argument} \code{indepTest} a function of the form
\code{\textit{CItest}(x, y, S, suffStat)} to test
...
myCItest <- function(x,y,S, suffStat) {
  if (length(S) == 0) {
    ...
    ## x. <- resid(lm(suffStat[,x] ~ suffStat[,S]))
    ## y. <- resid(lm(suffStat[,y] ~ suffStat[,S])) ## but more efficiently:
