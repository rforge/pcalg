...
\Abstract{ The \pkg{pcalg} package for \proglang{R} (\cite{citeR}) has two
  goals: Causal structure learning and estimation of causal effects from
  observational data. To this end, \pkg{pcalg} contains functions for
  estimating the causal structure using graphical models and functions for
  estimating intervention effects given an estimate of the causal
  structure. In this document, we will give a brief overview of the
  ...
  estimating intervention effects and graphical models using
  examples. Finally, we will illustrate some real world applications of the
  \pkg{pcalg} package.}
...
The objective of this paper is to introduce the R package \pkg{pcalg},
explain the range of functions on two simulated data sets and summarize
...
variable can also be visualized using a DAG where the arrows indicate
...
equivalence class. Every DAG in this equivalence class is equally likely to
represent the true causal structure.
...
variables of the underlying causal structure. It turns out
...
algorithm (``fast causal inference''; see \cite{SpirtesEtAl00}). This
algorithm was slightly extended and proven to be sound and complete in
...
algorithm. It turns out that the FCI is computationally infeasible for
large graphs. The RFCI algorithm (``really fast causal inference''; see
\cite{rfci}, is much faster than FCI. In some situations the output of RFCI
is slightly less informative, in particular with respect to conditional
...
as shown in Section \ref{sec:gm}, we can estimate the equivalence class, in which
the true causal DAG is. Taking this into account, we conceptually apply
...
\item[FCI/RFCI-algorithm:] Consistent in high-dimensional settings if the
...
estimating causal structures and graphical models. Second, it is intended
to provide a tool for estimating the causal effect among continuous
variables given observational data using a form of the do-calculus known as
back-door criterium when no causal structure is known and hidden or
selection variables are absent. For this, the causal structure is
...
effects when the causal structure is known or was estimated.
...
plot(pc.fit, main = "")
@
\caption{True DAG (left) and estimated skeleton (right) fitted on the simulated Gaussian data set
...
plot(skel.fit, main = "")
@
\caption{True DAG (left) and estimated skeleton (right) fitted on the simulated
...
plot(pc.fit, main = "")
@
\caption{True DAG (left) and estimated CPDAG (right) fitted on the
  simulated Gaussian
...
This function implements a generalization of the PC algorithm, in the sense
...
\caption{True data generating DAG (left) and estimated PAG
  (right). The estimated PAG using FCI and RFCI are the same. Variable 1 of the data generating DAG is latent.}
\label{fig:fci}
\end{figure}
...
This function is rather similar to \code{fci()}. However, it does not compute any
Possible-D-SEP sets and thus does not make tests conditioning on them. This
makes \code{rfci()} much faster than \code{fci()}. The orientation rules
for v-structures and 
rule 4 were modified in order to produce a PAG which, in the oracle
version, is guaranteed to have the correct ancestral relationships.
...
To illustrate this, consider the following example. We have simulated 10000
samples from seven Gaussian variables with a causal structure given on the
left of Fig. \ref{fig:ida}. We assume that the causal structure is unknown
and want to infer the causal effect of $V_2$ on $V_5$. First, we estimate
the equivalence class of DAGs that describe the conditional independence
relationships in the data, using the function \code{pc()} (see Section
\ref{sec:pc}).
...
In this section we summarize some applications in which the \pkg{pcalg} package
was used.
...
was to explore how graphical models and estimation of causal effects can be
used in the study of ICF data. For computations, the \pkg{pcalg} package
was used.

\subsection{Causal effects on time to flowering}
In CITE???, the objective was to find candidate genes that causally influence
the time to flowering in the plant Arabidopsis thaliane. This is of
interest, because fast growing crop plants lead to more efficient food
production. The data consisted of $n = 47$ observations of ``time to
flowering'' (the univariate response variable) and of expressions of $p = 21
326$ genes (the p-dimensional covariable), collected from wild-type
(non-mutated) plants. Based on these observational data, the authors
inferred the effects of a single gene intervention on the response of
interest (namely the ``time to flowering''), for each of the $p = 21 326$
genes.  The analysis was based on the \pkg{pcalg} and derived
functions. The result could be used to prioritize future biological
experiments, in particular for the situation here where ``fishing blindly''
for the best genetic modification would be extremely costly. The authors
also performed biological experiments for validation. As a result, 4 new
significant mutations were discovered showing a significant effect on the
``time to flowering''.

\subsection{Causal effects among genes}
In \cite{NatMethods10}, the authors aim at quantifying the effects of single gene
interventions on the expression of other genes in yeast, allowing for better
insights into causal relations between genes. With $n = 63$
observational data measuring the expression of $p = 5361$ genes, the goal
was to identify the largest intervention effects between all genes (in
total $p \cdot (p − 1) = 28 734 960$ effects). For the analysis, the
\pkg{pcalg} package was used.

The paper also includes data on 234 measurements of interventional experi-
ments, namely from 234 single-gene deletion mutant strains and for each of
them measuring the expressions of all the genes. Using this data, we know
the true causal effect in good approximation. We can then quantify how well
we can find the true intervention effects in the following way: We encode
the true large intervention effects as ``true'' effects and all others as
``false''. We then check in an ROC curve, how well the causal effects
estimated with \code{ida()} correspond to the true causal effects. For
comparison, the authors also used the (conceptually wrong) Lasso and
Elastic Net to identify the largest causal effects. In Figure
\ref{fig:yeast} one can see that \code{ida()} is clearly superior to the
alternative methods (and random guessing) in terms of identifying true
causal effects.

\begin{figure}[htb]
  \centering
  \includegraphics{Figure1FAT.pdf}
\caption{The largest causal effects found in experiments among yeast genes are
  identified much better with IDA than with Lasso, Elastic Net or random
  guessing. The figure is essentially taken from \cite{NatMethods10}.}
\label{fig:yeast}
\end{figure}

\section{Discussion}
Causal structure learning and the estimation of causal effects from
observational data has large potential. However, we don't propose this
method as a replacement for experiments. It should rather be a guide for
...
important to further validate them. We think that the \pkg{pcalg} package
makes a contribution to this important issue. Furthermore, methods relying
on fewer or milder assumptions (like RFCI) are desirable and need to be
developed further. Moreover, refined methods for assessing the uncertainty
of causal structures need to be developed.
